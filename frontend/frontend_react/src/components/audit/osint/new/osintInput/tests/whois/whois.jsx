...
const Whois = () => {
    ...
    function UrlHandlerLogs() {
        ...
        if (inputs.current) {
            ...
            instance.get('/pentesting/whois/?url=' + inputs.current.value)
                .then(res => {
                    setData(res);
                    setLoading("noload")
                }).catch(err => {
    ...
    return (<div >
        ...
        <h3 className={"osint_subtitle"}>Whois:</h3>
            <form style={{display: 'flex'}}>

                <input className={"Osint_input"} type={"search"} ref={inputs}
                       placeholder={"OpenPorts шабуылының осалдығын тексеру үшін URL мекенжайыңызды енгізіңіз"}/>
            ...
            {data.data ? <div>
                <div className="code_block--pentest">
                    <code>
                                <div className={"console_title"}>{JSON.stringify(data.data.whois)}:</div>
                            </code>
                </div>
            </div> : null}
        </div>);
